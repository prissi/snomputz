...
BOOLEAN	LadeBlock( LPBMPDATA pBmp, LPVOID pvPtr, LONG w, LONG ww, LONG h, int iBits, WORKMODE Mode, unsigned short uAdd, BOOLEAN InitRest )
{
	...
	if(  iBits==16  )
	{
		...
		{
			for(  x=0;  x<w;  x++  )
			{	// attention: Never put the next two lines into a single one!!!! (MS compiler error!!!!)
		...
		switch(str[0] )	// Um das Laden zu beschleunigen ...
		{
			...
			case 'X':
				if(  strstr(str,IMAGE_HEADER_CONTROL_X_AMPLITUDE)  )
				{
					...
					if(  str2[0]=='µ'  )	// micrometer
						dWx *= 1000.0;
					if(  str2[0]=='Å'  )	// Aangstroem
						dWx /= 10.0;
				}
				break;
...
BOOL	ReadSeiko( HFILE hFile, LPBMPDATA pBmp )
{
	 ...
	 *	Länge des Headers immer (?) 2944 Bytes, steht aber auch in der Datei
   *
...
BOOL	ReadRHK( HFILE hFile, LPBMPDATA pBmp )
{
	 ...
	 *	Das Format ist nur aufgrund von Try and Error entziffert worden!
	 *	Verwendet vorzeichenlose Intel-Notation.
	 *	Länge des Headers immer (?) 512 Bytes
	 ...
	 *	flag flag flag Wort (Breite) Wort (Höhe) Long (Dateilänge) flag
	 *
	...
	offset = _llseek( hFile, 0, 2 );	// Dateilänge
	_llseek( hFile, 0l, 0 );
...
BOOL	ReadOmicron( LPSTR datei, HFILE hParDatei, LPBMPDATA pBmp )
{
	HFILE			hMessDatei;
	WORKMODE	mode=NONE;
	LPSNOMDATA pSnom=&(pBmp->pSnom[0]);
	...
	// Header ist Def.-mäßig kleiner als 8096
	...
	*	Date												 : Datum Uhrzeit
	*
	*	Field X Size in nm           : Größe gesamt
	*	Field Y Size in nm           : Größe gsamt
	*	Image Size in X              : Punkte X
	*	Image Size in Y              : Punkte Y
	*	Increment X                  : Größe/Punkte in X
	*	Increment Y                  : Größe/Punkte in Y
	*	Scan Angle                   : ...
	*	X Offset                     : ...
	*	Y Offset                     : ...
	*
		...
		switch( str[0] )	// Um das Laden zu beschleunigen ...
		{
			...
			case 'T':
				...
				if(  strstr( str, "Topographic Channel")==str  )
				{
					...
					while(  iLen++<lHeaderLen  &&  *pcC++>=' '  );
					while(  iLen++<lHeaderLen  &&  *++pcC<=' '  );
					// Resolution, geht genauer anders ... :
					if(  mode==ERRO  )
					...
					while(  iLen++<lHeaderLen  &&  *pcC++>=' '  );
					while(  iLen++<lHeaderLen  &&  *++pcC<=' '  );
					// Hoffentlich in nm, wird noch nicht getestet!
					if(  mode==ERRO  )  
					...
					*pcC2 = 0;
					while(  iLen++<lHeaderLen  &&  *pcC++>=' '  );
					while(  iLen++<lHeaderLen  &&  *++pcC<=' '  );
					// Jetzt sollte pcC auf den Beginn des Filenamens zeigen
					lstrcpy( str, datei );
					pcC2 = strrchr( str, '\\' );
					if(  pcC2==0  )
						pcC2 = str;
...
#endif
	// Die unterschiedlichen Daten und die Binärdaten schreiben
	while(  i-->0  )
	{
...
BOOL	ReadDigital( HFILE hFile, LPBMPDATA pBmp )
{
	...
	// Einige Defaultwerte eintagen
	pBmp->pPsi.fRot = 0.0;
	// Header ist hoffentlich Def.-mäßig kleiner als 65536 (zumindest was den INHALT angeht ... )
	pcBuf = (LPBYTE)pMalloc( MAX_DI_HDLEN );
	if(  pcBuf==NULL  )
	{
		FehlerRsc( E_MEMORY );
	...
	*   \Scan size: f s      Scangröße (real) + Einheit (nm oder ~m=µm)
	...
	*   \Data length: x      und deren Länge
	*
	*	Neue Formate: (V4.4 und hoeher)
	*   \Scan size: f f s    Scangröße (real) x z Einheit (nm oder ~m=µm)
		...
		*pcC2++ = 0;
		...
		switch( str[1] )	// Um das Laden zu beschleunigen ...
		{
			...
			case 'D':
				...
				// Länge der Daten (ist eigentlich überflüssig ...)
			...
			case '@':	// New Header (wozu gab es eigentlich die alten Felder???)
				if(  strstr( str, "\\@Z magnify: ")==str  )
				{	// Beispiel: "\@Z magnify: C [2:Z scale] 0.3451050 "
				...
				if(  strstr( str, "\\@2:Image Data: ")==str  )
				{ // Art der Daten
			...
			case 'Z':
				// Z-Verstärkung (hoffentlich in nm)
				if(  lVersion  &&  strstr( str, "\\Z scale: ")==str  )
				{
					if(  Header==TOPO  )
					{
						...
						if(  *c1=='~'  )	// µm?
						{
							pSnom->Topo.fSkal *= 1000.0;	// nun in µm
							*c2++ = 'µ';
							*c1++;
						}
						while(  *c1>' '  )
					...
					if(  Header==ERRO )
					{
						...
						while(  *c1>' '  &&  *c1!='('  )
							c1 ++;
						if(  *c1=='~'  )	// µm?
							pSnom->Error.fSkal *= 1000.0;	// nun in µm
						while(  *c1>=' '  &&  *c1!='('  )
							c1 ++;
						c1++;
						lModuloSkal = atol((LPSTR)c1);
				...
				// Z-Verstärkung alt (hoffentlich!) in 1/100 nm
...
BOOL	ReadJena( HFILE hFile, LPBMPDATA pBmp )
{
	...
	pSnom->Topo.bSpecialZUnit = FALSE;
	pSnom->Topo.bShowNoZ = FALSE;
	lstrcpy( pSnom->Topo.strZUnit, "C/nm³" );
...
{
	DD_TAG HUGE *lpDD;
	/* DD_HEADER ist zu gross mit Visual C! */
	DD_HEADER	dh;
...
ASSERT( pBmp!=NULL  );
	...
	// Erst einmal davon ausgehen, dass die Datei nix sinnvolles enthält
...
ASSERT(  k<lMaxDD  );
				...
				{
													// Typ herausfinden
					case DFTAG_SDL: if(	 ReadLong(puPos)==ReadLong("Lumi")  )
														ThisMode = LUMI;
					...
					case DFTAG_SDD:	if(  Big2Little( *puPos )!=2  )
													{
														// Unbekannte Daten mit mehr als 2 Dimensionen
														// Könnten mal Spektrale Daten werden ...
														ThisMode = FALSE;
														j = lpDD[i].len;
														break;
													}
					...
					case DFTAG_SDU: // Einheiten feststellen:  (um oder µm) bzw. nm für Topo, sonst Lumi
														...
														if(  lstrcmpi((LPSTR)puPos,"nm")>0  )
															pBmp->pPsi.fH *= 1000.0;
														(LPSTR)puPos += 3;
														if(  lstrcmpi((LPSTR)puPos,"nm")>0  )		// nW wäre kleiner Null ...
															Bild.fSkal *= 1000.0;
													break;

					case DFTAG_CAL:	{	// Einige Klimmzuege um double von Mototrola nach INTEL zu konvertieren ...
														// Lohn ist die Skalierung der z-Achse in um (=µm) bwz nm (muss nachskaliert werden!)
														long	hilf[2];
ASSERT( sizeof(long)*2==sizeof(double) );
					...
					case DFTAG_SDM:	pBmp->pPsi.iMax = Big2Little( *puPos++ );	// Als Word!
													pBmp->pPsi.iMin = Big2Little( *puPos++ );
												break;

					case DFTAG_SD:	lDataStart = lpDD[k].offset;
					...
					case DFTAG_PSIHD:	// Auch Snomputz nutzt den binären Header
#if LEN_OK
			...
			// Alle möglichen Tags wurden durchlaufen, jetzt wird geladen!
...
WORD	CreateBildHDF( LPUCHAR hdf, LPLONG lPosition, DD_TAG HUGE **dd_tags, WORD ref, LPBMPDATA pBmp, LONG w, LONG h, LPBILD pBild, LPLONG *plStart, LONG lLen, WORD CompressionType )
{
	...
	*ndg++ = ref;
	...
	{	// Einige Klimmzuege um double von Mototrola nach INTEL zu konvertieren ...
		// Lohn ist die Skalierung der z-Achse in um (=µm)
		long	hilf[2];
...
// Komprimiert (so möglich) die Daten und gibt die neue Länge zurück
// Eingabe: Zeiger auf Ausgangsdaten, Länge (in Worten!), Flag, ob nach Bytes gewandelt werden soll
// Ausgabe: Länge der komprimierten Daten (0=erfolglos)
...
LONG	CompressData( LPUWORD *puData, LONG lLenSrc, WORD uMaxDaten, WORD *method )
{
	...
	lLen = (lLenSrc*nBits+7l)/8l;	// So lang wäre es maximal mit nBit-Decoding
	...
	if(  (*method&DELTA)==0  )
	{
		// Es läßt sich noch Platz durch Zusammenschieben gewinnen
		// Außerdem steht im Array pHilf noch nix sinvolles ...
		if(  nBits&7  )
		{
			// Bits packen
			LPUCHAR	pPtr=pHilf1;
...
ASSERT(	lLen == lLenSrc*2 );
	...
	if(  lNewLen<0  )
	{
		// Daten wurden länger ... war wohl nix!
		MemFree( pHilf );
		if(  nBits==16  &&  *method==0  )
		{
			// Ließen sich nicht packen!
			MemFree( pHilf1 );
			return 0;
		}
...
BOOLEAN	WriteHDF( LPBMPDATA pBmp, WORD iAktuell, WORKMODE what, LPSTR szDatei, BOOLEAN DoCompress )
{
	...
	// Als Long wäre es nicht portabel!!!
	dd ++;
	tag_nr ++;
	lPos += 4l;

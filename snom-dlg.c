...
/* Verwaltet Info-Dialog; dieser hat *KEINE* Button, 
   sondern wird mit jedem Return und jeden Linksklick beendet! */
...
BOOL WINAPI HandleBrowseDialog(HWND hdlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		...
		if(  wParam==OVER_LIST  )
		{
			...
			switch( SortOrder )
			{
				case OVER_DATE:	return CompareFileTime(  &(pItem1->FT), &(pItem2->FT)  );
				case OVER_NUMBER:	return ((pItem1->iNr>pItem2->iNr)?-1:1);
				case OVER_NAME:		return lstrcmpi(pItem1->strName,pItem2->strName);
			}
			return 0;
		}
		...
		if(  wParam==OVER_LIST  )
		{
			...
			if(  !pItem->bLoaded  )
			{
				...
				if(  pBmp->pSnom[0].Topo.puDaten!=NULL  )
				{
					...
					if(  flatten  ) {
						if(  pBmp->pSnom[0].Topo.uMaxDaten>8096u  ) BildCalcConst( &(pBmp->pSnom[0].Topo), 128, 128, '/', pBmp->pSnom[0].Topo.uMaxDaten/8096.0, FALSE );
	...
	switch (message)
	{
		case WM_INITDIALOG:
			...
			if(  pBmp!=NULL  )
			{
				...
				switch(  pBmp->Links  )
				{
					case TOPO:	SendDlgItemMessage( hdlg, ANSICHT_LINKS, CB_SELECTSTRING, (WPARAM)-1, (LPARAM)(LPSTR)STR_TOPO );
						break;
					case ERRO:	SendDlgItemMessage( hdlg, ANSICHT_LINKS, CB_SELECTSTRING, (WPARAM)-1, (LPARAM)(LPSTR)STR_ERROR );
						break;
					case LUMI:	SendDlgItemMessage( hdlg, ANSICHT_LINKS, CB_SELECTSTRING, (WPARAM)-1, (LPARAM)(LPSTR)STR_LUMI );
						break;
				}
				switch(  pBmp->Rechts  )
				{
					case TOPO:	SendDlgItemMessage( hdlg, ANSICHT_RECHTS, CB_SELECTSTRING, (WPARAM)-1, (LPARAM)(LPSTR)STR_TOPO );
						break;
					case ERRO:	SendDlgItemMessage( hdlg, ANSICHT_RECHTS, CB_SELECTSTRING, (WPARAM)-1, (LPARAM)(LPSTR)STR_ERROR );
						break;
					case LUMI:	SendDlgItemMessage( hdlg, ANSICHT_RECHTS, CB_SELECTSTRING, (WPARAM)-1, (LPARAM)(LPSTR)STR_LUMI );
						break;
				}
			}
...
#if 0
			SetDlgItemInt( hdlg, ANSICHT_XY_EDIT, fXYAngle*90.0+0.5, FALSE );
			SetDlgItemInt( hdlg, ANSICHT_Z_EDIT, fZAngle*90.0+0.5, FALSE );
...
BOOL WINAPI UnitDialog (HWND hdlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		...
		switch (wParam)
		{
			...
			case IDOK:
			{
				double	faktor=1.0, fSkalX, fSkalY, fSkalZ, fIntens;
				LPSNOMDATA pSnom;
...
//					if(  (pDaten = (LPUWORD)GlobalLock( GlobalPtrHandle( pBmp->pSnom[iAkt].Topo.puDaten ) ))==NULL  )
					...
					if(  pQuadrate!=NULL  )
					{	// Ein float-Array mit den Quadraten für die RMS-Berechnung bereitstellen
						unsigned long k;
...
int WINAPI FFTDialog (HWND hdlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		case WM_INITDIALOG:
			...
			while(  iSize<lW  )
				iSize *= 2;
			fFSpan = pBmp->pSnom[0].w*pBmp->pPsi.fLinePerSec;	// Obere Grenzfrequenz *2.0, da hin +rück, /2.0, da FFT nur bis halbe Frequenz ...
			if(  fFSpan==0  )
				fFSpan = lW;

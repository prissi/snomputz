	...
	while( DspThreadHandle != NULL ) {
		...
		if( ( *ReadMailboxTerminate )( 0, 0, &l, TRUE ) ) {
			switch( l ) {
				...
				case STARTLINE:
					if( bReadScan )	{                                         /* Lese eine Scanzeile */
						LONG lData;
						for( l = 0;  l < lScanW;  l++ )	{
							lData = ( *ReadMailbox )( 0, 0 );
							if( pData1 != NULL ) {
		...
		case WM_COMMAND:
			switch( LOWORD( wParam ) ) {
				...
				case MESS_APPROACH_APPROACH:
					...
					MotorThreadHandle = CreateThread( NULL, 0, MotorThread, (LPVOID)&Snom_MotorControl, 0, &MotorThreadHandleID );
					break;
	...
	switch( message ) {
		...
		case WM_INITDIALOG:
		{
			if( DspThreadHandle == NULL ) {
				DspThreadHandle = CreateThread( NULL, 0, DspThread, (LPVOID)hDlg, 0, &DspThreadHandleID );

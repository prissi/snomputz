...
{
	char *maxmap = (char *)calloc( width*height, 1 );

	int maxSortingError = 0;

	int nMax = 0;
	int nResult = 0;
	XYZ_COORD *maxpts = (XYZ_COORD *)calloc( (width*height), sizeof(XYZ_COORD) );
	XYZ_COORD *pResult = 0;
	XYZ_COORD *pList = 0;
...
//				if(  check_x>=0  &&  width>check_x  &&  check_y>=0  &&  height>check_y  ) {
	...
	pList = (XYZ_COORD *)calloc( width*height, sizeof(XYZ_COORD) );	// temporary
	pResult = (XYZ_COORD *)calloc( nMax+1, sizeof(XYZ_COORD) );	// final ... );

...
BOOLEAN	WriteDib( LPSTR szFileName, LPBITMAPINFO lpDib, LPBYTE lpDaten )
{
	...
	{
		// Run length encoded
		LONG	x=0, y=0, w=lpDib->bmiHeader.biWidth; h=lpDib->bmiHeader.biHeight;
...
BOOL CALLBACK EnumRedrawAll( HWND hwnd, LPARAM flags )
{
	...
	if(  flags&SLIDER  )
	{
		...
		if(  pBmp->bIsScanLine  )
		{
			...
			// Platz für Scanline dazurechnen
...
#define	MAX_SCALE	1e10	// oder auch hierüber
...
// Berechnet die Anzahl signifikanter Stellen nach dem Komma für Achsenskalierung
...
void	CalcDibSize( HDC hdc, LONG cxDib, LONG cyDib, LPRECT pCoord, BOOL Show3D, BOOL ShowScanline )
{
	...
	// Ausmaße der 3D-Bitmap berechnen, ansonsten ist Hoehe und Breite cxDib/cyDib
	if(  Show3D  )
	{
...
//	Farben kleiner min oder größer max werden abgeschnitten
							...
							WORD min, WORD max, float fColorFaktor, WORD StartColor, LPBILD pBild )
{
	...
	// Z-Weite nur mal 256, da sonst evt. Überlaufprobleme!
...
BOOL	RecalcCache( LPBMPDATA pBmp, BOOL Bitmaps, BOOL DontEmulContrast )
{
	...
	char	str[16];	// für Zahlen


	// Zuerst Pointer für rechte und linke Seite holen
	...
	// Verhältnis zuweisen; Minimum sind 15 Farben!
	// Dabei wird (noch?) nicht gecheckt, ob evt. Farben doppelt genutzt werden könnten
	if(  pLinks  )
	{
	...
	if(  usedcol>250l  )
	{
		...
		// Wert >1.0 (= mehrere Farben für einen Bitwert) sind sinnlos!
		if(  maxlinkscol>1.0  )
			maxlinkscol = 1.0;
		if(  maxrechtscol>1.0  )
			maxrechtscol = 1.0;
	...
	// Endlich die Anzahl der nötigen Farben berechnen
	...
	//****  Ab hier werden die Ausmaße berechet
	w = h = 0;
	SelectObject( hDC, CreateFontIndirect( &lf ) );  // Für alle Rechnungen
	// LinksX==0  (analog für RechtsX) heißt: Nicht existent!

	pBmp->rectLinks.left = pBmp->rectLinks.right = pBmp->rectRechts.left = pBmp->rectRechts.right = 0;
	pBmp->rectLinks.top = pBmp->rectRechts.top = 0;		// Doppelte Höhe der Überschriften

	if(  pLinks  )
	{
		// Ausmaße der Skala berechnen
		gcvt( CalcIncrement( 3, 9, pLinks->uMaxDaten*pLinks->fSkal )*5.0+0.23, 4, str );
		GetTextExtentPoint( hDC, str, lstrlen(str), &size );
		skala_w = size.cx;
		skala_h = size.cy*3+8;
		GetTextExtentPoint( hDC, "Ty", 2, &size );
		size.cy *= 2;	// Doppelte Höhe der Überschriften
	...
	// Platz für Scanline evt. schon mal dazurechnen
	...
	// Text und Grafik für Skalierung werden eingefügt
	...
	// Ansonsten werden die Farben kastriert!
	{
		...
		// Zuerst das Vorgeplänkel, anlegen von Memory-DC, usw.
		...
		// und nun die mühsam gewonnen Daten wieder in ein Bild wandeln ...
	...
	if(  pLinks  )
	{
		...
		if(  !pLinks->bPseudo3D  ||  maxlinks<=2  )
		{
			...
			if(  !pLinks->bShowNoZ    )
			{
				...
				for(  y=0;  y<pSnom->h;  y++  )
				{
					// -1 bei y, da die Bitmap nur bis pSnom->h-1 läuft (for y<h!)
					for(  x=0;	x<8;	x++  )
						if(  !DontEmulContrast  )
...
#ifdef SIMPLE_CONTOUR
						else if(  pLinks->bKonturen  &&  pSrc[x]%pLinks->uKontur<=pLinks->uKonturToleranz  )	// so einfach wären Konturen einzufügen
							pDest[x] = 4;
#endif
						else if(  pLinks->bModuloKonturen  )
//						pDest[x] = (BYTE)(startcol+(pSrc[x]%pLinks->Modulo)*maxlinkscol+0.5);
...
//						pDest[x] = (BYTE)(startcol+(pSrc[x]*maxlinkscol+.5));
					}
					pDest += ww;
					pSrc -= pSnom->w;
				}
...
//						pDest[x] = (BYTE)(startcol+((long)(pSrc[x]%pRechts->Modulo)<<10l)/lScaleFactor);
						else if(  pSrc[x]>=maxrechts  )
							pDest[x] = endcol;
						else
//						pDest[x] = (BYTE)(startcol+(pSrc[x]*maxrechtscol+.5));
...
void	CopyDibToDC( HDC hDC, LPBITMAPINFO pDib, LPBYTE pDest, RECT xywh )
{
	...
	if(  xywh.bottom<0  )
//		StretchBlt( hDC, xywh.left, xywh.top+xywh.bottom, xywh.right, -xywh.bottom, hMem, 0, 0, xywh.right, -xywh.bottom, SRCCOPY);
...
{
	HDC			hMem;
	HGDIOBJ	hOldFont, hBitmap, hOldBitmap;
	RECT		dibXywh;
...
//	POINT				pPoint;
	...
	if(  bIgnoreRect  )
	{
		...
		xywh->left = 0; xywh->top = 0; 
#ifdef _MSC_VER
		pSnom = &(pBmp->pSnom[iAkt]); // MSVC braucht diese Zeile! Compilerfehler!!!
#endif
		xywh->right = pSnom->w*fZoom; xywh->bottom = pSnom->h*fZoom;
...
pBmp->rectPlot.bottom = xywh->bottom-xywh->top;
	...
	{
		if(  !pLinks->bPseudo3D  )
		{// draw vertical Axis and advance
...
void	ScanLinePlot( HDC hdc, LPBMPDATA pBmp, LPBILD pBild, double fZoom, PROFILFLAGS wScanlineModus, LPUWORD puTemp, LPFLOAT pfTemp[4], int iSize, BOOLEAN bNeuZeichen )
{
	...
	float		fWeite[MAX_SCANLINE], fMaxWeite, fMaxHoehe, fMaxMax=-1e20, fMaxMin=1e20, f;
...
void	DrawScanLinePlot( HDC hdc, LPBMPDATA pBmp, double fScale, BOOLEAN bWhiteOut )
{
	...
	// Erst mal inititalisieren für den Fall: Keine Scanline
...
//	pBmp->rectFenster.bottom = pBmp->pDib->bmiHeader.biHeight;
	...
	lf.lfHeight /= fScale;
	hOld = SelectObject( hdc, CreateFontIndirect( &lf ) );  // Für alle Rechnungen
	SetMapMode( hdc, j );
	lf.lfHeight = i;
...
// Alte Routine vom Mai '97, funktioniert immer noch!
